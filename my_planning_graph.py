...
class PlanningGraph():
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for e1 in action1.effect_add:
            for e2 in action2.effect_rem:
                if e1 == e2:
                    return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        action1 = node_a1.action
        action1effect = action1.effect_add + action1.effect_rem
        action1precon = action1.precond_pos + action1.precond_neg
        action2 = node_a2.action
        action2effect = action2.effect_add + action2.effect_rem
        action2precon = action2.precond_pos + action2.precond_neg
        for e1 in action1effect:
            for e2 in action2precon:
                if e1 == e2:
                    return True
        for e3 in action1precon:
            for e4 in action2effect:
                if e3 == e4:
                    return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        action1parents = node_a1.parents
        action2parents = node_a2.parents
        for p1 in action1parents:
            for p2 in action2parents:
                if p1.is_mutex(p2):
                    return True
        return False
